...
## Malatavadi submission method

This repo contains a focused Malatavadi-only attempt in `solve.py`. It keeps each official plot
shape unchanged and searches for a small translation that better aligns the plot outline with the
visible field evidence.

Run:

```bash
python -m uv sync
UV_CACHE_DIR=.uv-cache python -m uv run solve.py data/12429_malatavadi_chandgad_kolhapur
```

On PowerShell, set the local uv cache first:
...
For each plot, the solver:

- samples points along the official polygon boundary;
- tries translations within 20 metres of the official position;
- scores each translated outline against `boundaries.tif` and satellite-image edge strength from
  `imagery.tif`;
- penalizes larger shifts;
- uses the map-area / recorded-area ratio as a risk signal;
- returns `corrected` only when the best shift clearly beats the original;
- otherwise returns `flagged` with the official geometry unchanged.

### Confidence

Confidence is only emitted for corrected plots. It combines:

- edge-signal strength at the predicted outline;
- margin over the original outline;
- how small the shift is;
- whether the recorded area agrees with the drawn map area.

Low-confidence cases are flagged rather than weakly corrected. This is deliberate: the assignment
scores calibration and restraint, so the method prefers fewer trustworthy moves over broad
overclaiming.

### Known limits

This v1 is translation-only. It does not rotate or reshape polygons, and it does not learn a model
from the hidden truths. It should do best where the official shape is roughly right but locally
shifted. It should flag plots where the record/shape disagree, boundaries are hidden by trees or
buildings, or the edge raster is ambiguous.

### Video outline

1. Problem: official cadastral boundaries are shifted relative to real fields.
2. Scope: Malatavadi only, because the downloaded bundle is local and the parcels are dense enough
   to test confidence.
3. Method: translation search scored by boundary hints plus image gradients.
4. Confidence: based on signal, margin, shift size, and area sanity.
5. Limitations and next work: add rotation/reshape, neighborhood consistency, and cross-village
   validation.
